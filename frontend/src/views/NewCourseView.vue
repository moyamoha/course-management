    ...
    <v-form class="mt-6 px-6 creation-form" ref="form" @submit="onSubmit">
      ...
      <v-row align-content="center" class="mt-1">
        ...
        <v-col class="d-flex justify-end">
          <v-btn-toggle dense borderless rounded color="deep-purple accent-4" v-model="newCourse.language">
      ...
      <v-col md="6" sm="12" class="px-0 mt-4" v-if="courseIsFinished">
        <v-select v-if="isHyvHyl" :items="resultsHyvHyl" label="Course result" dense outlined v-model="newCourse.result"></v-select>
        <v-select v-else :items="results0To5" label="Course result" dense v-model="newCourse.result"></v-select>
      </v-col>
      <v-divider class="my-4"></v-divider>
      <v-card-text class="px-0 mt-2 d-flex">
        <v-btn color="purple" class="mr-2" outlined @click="onCancel">Cancel</v-btn>
