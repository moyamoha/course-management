		...
		<v-dialog width="300" v-model="dialog">
			<template v-slot:activator="{ on, attrs }">
				...
				<v-btn
					...
					outlined
					small
					v-else-if="course.state === 'notStarted'"
					>Start course</v-btn
				>
			</template>
			<v-card class="pa-3">
				...
				<v-select
					...
					dense
					class="mx-2"
					placeholder="Result of course"
					v-model="course.result"
					:items="availableResults"></v-select>
				<v-btn outlined color="primary" class="mx-2" @click="markCourseAsDone"
					>Ok</v-btn
				>
			</v-card>
		</v-dialog>
...
<script>
	...
	export default {
		...
		computed: {
			availableResults() {
				return this.course.scale === 'Hylätty-Hyväksytty'
					? this.resultsHyvHyl
					: this.results0To5
			},
		},
